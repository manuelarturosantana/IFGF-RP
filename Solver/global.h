...
double inline EQUAL_TOL = 1.0e-12;

// Beta double layer close

double inline beta_double_layer_close(const double p1x, const double p1y, const double p1z,
...
double inline fun_v(double p, double tau)
{
...
double inline fun_dv(double p, double tau)
{
...
double inline fun_w(double p, double tau) 
{        
...
double inline fun_dw(double p, double tau)
{
...
void inline cheb_evals(const std::vector<double>& x, std::vector<std::complex<double>>& evals, int N, int M)
{
...
void inline golden_search(const std::function<double (double, double)>& f, int max_iter, double tol, double u_a, double u_b, double v_a, double v_b, double& sol_x, double& sol_y)
{
...
std::vector<double> inline barycentric_weights(const std::vector<double>& nodes)
{
...
void inline lagrange_interpolation_2D(const std::vector<double>& xNodes, const std::vector<double>& yNodes,
                               ...
                               double * solution)
{
    ...
    for (int i = 0; i < P; i++) {
        for (int j = 0; j < N; j++) {
            ...
            if (std::abs(x[i] - xNodes[j]) < EQUAL_TOL) {
                idx_equal_x[i] = j;
                break;
            }
            ...
            vec_x[i * N + j] = xWeights[j] / (x[i] - xNodes[j]);
    ...
    for (int i = 0; i < Q; i++) {
        for (int j = 0; j < M; j++) {
            ...
            if (std::abs(y[i] - yNodes[j]) < EQUAL_TOL) {
                idx_equal_y[i] = j;
                break;
            }
            ...
            vec_y[i * M + j] = yWeights[j] / (y[i] - yNodes[j]);
    ...
    for (int i = 0; i < P; i++) {
        ...
        if (idx_equal_x[i] != -1) {
            ...
            std::fill(vec_x.begin() + i*N, vec_x.begin() + (i+1)*N, 0);
            vec_x[i*N + idx_equal_x[i]] = 1.0;
            lx[i] = 1.0;
    ...
    for (int i = 0; i < Q; i++) {
        ...
        if (idx_equal_y[i] != -1) {
            ...
            std::fill(vec_y.begin() + i*M, vec_y.begin() + (i+1)*M, 0);
            vec_y[i*M + idx_equal_y[i]] = 1.0;
            ly[i] = 1.0;
...
void inline print_max_RSS() 
{

    rusage tmp_usage;
    const int err = getrusage(RUSAGE_SELF, &tmp_usage);
    if (err != 0)
        std::cout << "Memory measurement failed." << std::endl;
    else
        std::cout << "Memory used = " << static_cast<double>(tmp_usage.ru_maxrss)/(1<<20) << " GB" << std::endl;
